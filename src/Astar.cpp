...
std::vector<std::pair<int, int>> Astar::getpath(std::pair<int, int> begin, std::pair<int, int> end) {
    std::vector<std::pair<int, int>> direction = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};

    Node* beginNode = new Node(begin.first, begin.second, 0, calc_h(begin, end), nullptr);
    ...
    while (!OpenList.empty()) {
        ...
        for (size_t i = 0; i < direction.size(); i++) {
            ...
            Node* openNode = new Node(next.first, next.second, calc_g(next.first, next.second, parent->x, parent->y),
